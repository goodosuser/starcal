...
class EventHistoryDialog(gtk.Dialog):
	...
	def treeviewCursorChanged(self, treev):
		self.updateViewType()
	...
	def updateTableViewType(self, viewType, hashBefore, hashAfter):
		...
		if viewType == "Change Table":
			if hashBefore != "":
				...
				for key, (valueBefore, valueAfter) in diff.items():
					trees.append(["M", key, valueBefore, valueAfter])
	...
	def extractChangeSummary(self, diff):
		...
		return _("%d parameters") % len(diff)
